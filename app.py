...
# Chroma client (we will force DuckDB backend)
import chromadb
...
if api_key:
    ...
    if uploaded_files:
        ...
        documents = []
        for uploaded_file in uploaded_files:
            temp_path = "./_temp_upload.pdf"  # reused per loop; fine for sequential load
        ...
        # --------- Chroma with DuckDB backend (NO sqlite) ---------
        # This avoids the sqlite3 >= 3.35 requirement on Streamlit Cloud.
        chroma_client = chromadb.Client(
            chromadb.config.Settings(
                chroma_db_impl="duckdb+parquet",
                persist_directory=".chroma",  # folder to persist your embeddings
            )
        )

        # Build / load vector store
        # NOTE: langchain_chroma expects embedding_function + client
        vectorstore = Chroma.from_documents(
            documents=splits,
            embedding=embeddings,
            client=chroma_client,
